...
void Sprite_Draw(TextureIndex texture_index, float dx, float dy, int tx, int ty, int tw, int th, float cx, float cy, float sx, float sy,float rotation )//ｃｘｃｙ中心、ｓｘｓｙサイズ、rotationラジアン
{
	LPDIRECT3DDEVICE9 pDevice = GetD3DDevice();
	if (!pDevice)return;
	...
	for (int i = 0; i < 4; i++) {
		matA11 = matBase[i] * matScale*matRot*matTrans;
		px[i] = matA11._41;
		py[i] = matA11._42;
	}
